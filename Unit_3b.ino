...
SparkFun_VL53L5CX sensorRL;  // Create an instance of the sensor object
...
void setup() 
{
  ...
  sensorRL.setResolution(imageWidth * imageWidth);  // Set resolution to 4x4 grid
  sensorRL.setRangingFrequency(15);  // Set frequency for measurements (15 Hz)
  sensorRL.startRanging();  // Start collecting distance measurements
}
...
void loop() 
{
  ...
  if(sensorRL.isDataReady())  // Check if new data is ready from the sensor
  {
    if(sensorRL.getRangingData(&measurementData))  // Fetch the latest measurement data
    {
...
void sendToFirebase(int motionStatus) 
{
  ...
  String url = "/3b/LiDAR.json?auth=" + String(firebaseSecret);  // URL to send data to Firebase with the secret for authentication
